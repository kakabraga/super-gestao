<div class="border border-sky-600 rounded-xl mt-6 p-8 bg-white shadow-md w-full max-w-md">
    ...
    @endif
    <form action="{{ $auth === 'login' ? route('site.login.submit') : route('site.register.submit') }}" method="post"
        class="space-y-5">
        @csrf
        ...
        @if($auth == 'register')
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Digite seu nome" class="w-full rounded-md border border-sky-600 px-3 py-2
        ...
        <div>
            ...
            <input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="Digite seu email" class="w-full rounded-md border border-sky-600 px-3 py-2
                       text-sm text-sky-900
                       focus:border-sky-500 focus:ring-2 focus:ring-sky-500/30
                       outline-none transition" required />
        ...
        <div>
            ...
            <input type="password" id="password" name="password" placeholder="Digite sua senha" class="w-full rounded-md border border-sky-600 px-3 py-2
                       text-sm text-sky-900
                       focus:border-sky-500 focus:ring-2 focus:ring-sky-500/30
                       outline-none transition" required />
 ...
 @error('login_error')
        ...
        @if($auth == 'register')
            <div>
                ...
                <input type="password" id="password_confirmation" name="password_confirmation"
                    placeholder="Confirme sua senha" class="w-full rounded-md border border-sky-600 px-3 py-2
                           text-sm text-sky-900
                           focus:border-sky-500 focus:ring-2 focus:ring-sky-500/30
                           outline-none transition" required />
        ...
        @if($auth == 'login')
            <div class="border-t pt-4 mt-4 text-center">
                <span class="text-sm font-medium text-sky-900">
                    ...
                    <a href="{{ route('site.login.submit', ['auth' => 'register']) }}"
                        class="text-indigo-900 hover:underline ml-1">
                        Clique aqui
                    </a>
                </span>
            </div>
        ...
        @if($auth == 'register')
            <div class="border-t pt-4 mt-4 text-center">
                <span class="text-sm font-medium text-sky-900">
                    Já possui uma conta?
                    <a href="{{ route('site.login.submit', ['auth' => 'login']) }}"
                        class="text-indigo-900 hover:underline ml-1">
                        Clique aqui
                    </a>
                </span>
